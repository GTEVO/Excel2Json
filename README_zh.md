...
#### 目标框架net9，项目代码没有特殊依赖，可以修改为其他版本。
#### 发布二进制可运行文件时，启用ReadyToRun编译可以提高性能，文件体积会稍微大些。
#### Clone到本地后可以直接用VsCode打开，安装C#扩展，直接运行调试，会将Data目录下的测试文件转化为json文件。可以在Excel被办公软件打开的情况下运行转换工具。
